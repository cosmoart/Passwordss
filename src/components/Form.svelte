<script>
	...
	function setLocalStorage(){
		localStorage.setItem('cosmoPasswordConfig', JSON.stringify({password_length, password_charset, catchyMode}))
	}
	...
	function handleRange(e){
		...
		setLocalStorage()
		catchyMode ? createCatchyPassword() : createPassword()
	}
...
<form class="flex flex-col gap-2">
	<div class="password_container relative after:absolute after:h-[3px] after:transition-all" style={`--password_width: ${password_strength.width}; --password_color: ${password_strength.color}`}>
		<output class="bg-white block resize-none text-black w-full h-11 p-2 pl-3 pr-6 text-[15px] leading-[25px] truncate">
			{#if catchyMode}
				{password_catchy}
			{:else}
				{#each password as char}
					<span class="transition-colors charset_{[CHARSET_UPERRCASE, CHARSET_LOWERCASE, CHARSET_NUMBER, CHARSET_SPECIAL].findIndex((item) => item.includes(char)) + 1}">{char}</span>
				{/each}
			{/if}
		</output>
		<button type="button" title="Other Password" on:click={() => catchyMode ? password_catchy = createCatchyPassword() : createPassword()} class="bg-white p-[10px] font-bold uppercase scale-90 absolute top-1/2 -translate-y-1/2 right-0 active:-rotate-180 hover:opacity-75 transition-all rounded-full">
	...
	<fieldset class="flex gap-2 items-center">
		...
		/>
		<input type="range" min={password_min_length} max={password_max_length} value={password_length}
	...
	<div class="relative h-8 overflow-hidden">
			<fieldset class={`flex gap-2 justify-between mt-2 absolute transition-all w-full ${catchyMode ? "translate-x-[-110%]" : ""}`}>
				<legend class="contents">Characters used:</legend>
				{#each Object.entries(INPUTS_CHARSET) as [Title, Value], i}
					<label class="cursor-pointer" title={Title} on:mouseover={() => handleColorChange(`.charset_${i + 1}`)} on:mouseout={() => handleColorChange(`.charset_${i + 1}`, 'remove')} on:focus={() => handleColorChange(`.charset_${i + 1}`)} on:blur={() => handleColorChange(`.charset_${i + 1}`, 'remove')}>
						<input type="checkbox" value={Value[1]} on:change={handleCheck} checked={password_charset.includes(Value[1])}
		...
		<CatchyPassword bind:password_catchy={password_catchy} {password_length} {catchyMode}/>
	</div>

	<div class="switchMode -bottom-3 -right-3 absolute hover:scale-105 active:scale-95 cursor-pointer h-8">
		<button type="button" id="witchMode" on:click={handleSwitchMode} class=" bg-gradient-to-r from-purple-600 to-blue-600 rounded-full bg-[length:200%_100%] w-8 h-8 bottom-0 right-0 hover:scale-105 active:scale-95 transition-transform absolute"><img src={icon_arrow_right} alt="" class={`mx-auto w-3/5 transition-transform ${catchyMode ? "rotate-180" : ""}`}></button>
		<label for="witchMode" class="bg-[#35354e] w-0 absolute right-0 transition-all overflow-hidden rounded-full h-full px-4 text-[15px] inline-block cursor-pointer py-1 -z-10" style={`--maxWidth: ${catchyMode ? "11rem" : "12.5rem"}`}>
